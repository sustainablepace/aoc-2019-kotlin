...
enum class Instruction_(val id: Int, val numParameters: Int) {
    PLUS(1, 3),
    TIMES(2, 3),
    READ(3, 1),
    WRITE(4, 1),
...
val instructions = Instruction_.values().associateBy { it.id }
...
fun CoroutineScope.launchComputer(program: String, input: ReceiveChannel<Int>, output: SendChannel<Int>) {
    ...
    launch {
        while (true) {
            ...
            if (instruction == Instruction_.HALT) {
                break
            } else {
                ...
                val jump = when (instruction) {
                    Instruction_.PLUS -> null.also { params[2].write(params[0].read() + params[1].read()) }
                    Instruction_.TIMES -> null.also { params[2].write(params[0].read() * params[1].read()) }
                    Instruction_.READ -> null.also { params[0].write(input.receive()) }
                    Instruction_.WRITE -> null.also { output.send(params[0].read()) }
                    Instruction_.JMP_IF_TRUE -> if (params[0].read() != 0) params[1].read() else null
                    Instruction_.JMP_IF_EQ -> if (params[0].read() == 0) params[1].read() else null
                    Instruction_.LESS_THAN -> null.also { params[2].write(if (params[0].read() < params[1].read()) 1 else 0) }
                    Instruction_.EQUALS -> null.also { params[2].write(if (params[0].read() == params[1].read()) 1 else 0) }
